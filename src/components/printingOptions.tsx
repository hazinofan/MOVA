...
import {
    ...
    MapPin,
    BadgeCheck,
    type LucideIcon, Award, Banknote, Box, Maximize2, Truck
} from "lucide-react";
...
type SizeRow = { label: string; values: (string | number)[] };

const SIZE_COLUMNS = ["S", "M", "L", "XL", "2XL", "3XL", "4XL"];

const SIZE_ROWS: SizeRow[] = [
    { label: "Width, in", values: [18.25, 20.25, 22.0, 24.0, 26.0, 27.75, 29.75] },
    { label: "Length, in", values: [26.62, 28.0, 29.37, 30.75, 31.62, 32.5, 33.5] },
    {
        label: "Sleeve length from center back, in",
        values: [16.25, 17.75, 19.0, 20.5, 21.75, 23.25, 24.63],
    },
    { label: "Size tolerance, in", values: [1.5, 1.5, 1.5, 1.5, 1.5, 1.5, 1.5] },
];
...
function CareIcon({ kind }: { kind: "wash" | "bleach" | "dry" | "iron" | "dryclean" }) {
    ...
    switch (kind) {
        case "wash":
            return (
                <svg {...common}>
                    ...
                    <path d="M9 7V5.8C9 4.8 9.8 4 10.8 4h2.4C14.2 4 15 4.8 15 5.8V7" stroke="currentColor" strokeWidth="1.6" />
                </svg>
            );
        case "bleach":
            return (
                <svg {...common}>
                    <path d="M12 4l7 16H5l7-16Z" stroke="currentColor" strokeWidth="1.6" />
        ...
        case "iron":
            return (
                <svg {...common}>
                    <path d="M6 16h12v-4c0-2.2-1.8-4-4-4H8c-1.1 0-2 .9-2 2v6Z" stroke="currentColor" strokeWidth="1.6" />
...
export default function FulfillmentOptionCard({
    ...
    const features = Array.isArray(details?.keyFeatures) ? details.keyFeatures : [];

    const keyFeatures = Array.isArray(details?.keyFeatures) ? details.keyFeatures : [];
                                                ...
                                                <table className="min-w-[860px] w-full border border-black/10">
                                                    ...
                                                    <tbody>
                                                        {SIZE_ROWS.map((row, idx) => (
                                                            <tr key={row.label} className={idx % 2 === 0 ? "bg-[#f7f7f3]" : "bg-white"}>
