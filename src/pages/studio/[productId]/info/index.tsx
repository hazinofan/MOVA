...
import type { ProductType, ProductColor, ProductConfig } from "@/lib/products";
...
export default function ProductInfoPage() {
  ...
  const thumbs: Thumb[] = useMemo(() => {
    ...
    const fromViews: Thumb[] =
      product.views?.map((v: any) => ({
        src: typeof v.mockup === "function" ? v.mockup(defaultColor) : v.mockup,
        alt: v.label ?? "View",
      })) ?? [];
  ...
  const bullets: string[] = (product as any)?.bullets ?? [
  ...
  const isLoading = !router.isReady || !pid || loadingProduct;
  const notFound = router.isReady && !!pid && !loadingProduct && !product;

  const fulfillment = useMemo(() => {
    if (!product) return null;

    // You can define these in products.ts per product if you want
    const shipFrom = (product as any)?.shipFrom ?? {
    ...
    const colorsCount =
      ...
      (((product as any)?.colors?.length ?? 0) || 10);
    ...
    const sizesLabel =
      ...
      (Array.isArray((product as any)?.sizes)
        ? `${(product as any).sizes[0]} – ${
            (product as any).sizes[(product as any).sizes.length - 1]
          }`
        : "S – 4XL");
    ...
    const printAreas = (product as any)?.printAreas ??
      (product as any)?.areas
        ?.filter((a: any) => a.id === "front" || a.id === "back")
        ?.map((a: any) => ({
          id: a.id,
                    ...
                    {thumbs.length ? (
                      <>
                        ...
                        <button
                          ...
                          disabled={
                            thumbs.findIndex((t) => t.src === active?.src) <= 0
                          }
            ...
            <aside className="space-y-5">
              <div className="p-6">
                ...
                <div className="mt-6 border border-black/10 bg-emerald-50 p-5">
                  <div className="text-3xl flex flex-row gap-1 text-neutral-700 items-end">
                    {" "}
                    MOVA{" "}
                    <span
                  ...
                  <div className="mt-3 text-base text-neutral-600">
                    Tip: Use PNG with transparent background for the cleanest
                    print.
                  </div>
                </div>
              </div>
